...
func GetMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if movieCollection == nil {
			log.Println("❌ movieCollection is nil — check your DATABASE_NAME in .env")
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Database collection not initialized."})
			return
		}

		cursor, err := movieCollection.Find(ctx, bson.D{})
		if err != nil {
			log.Println("❌ Error finding movies:", err)
